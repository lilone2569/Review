...
3线：①主机只给从机发送命令，从机不需要回复数据的时候(无MISO) ②主机只读取从机的数据，不需要给从机发送指令(无MOSI)

2线：当一个主机一个从机的时候，从机的片选有时可以固定为有效电平而一直处于使能状态，那么CS就可以不要；
此时如果再加上主机只给从机发送数据，那么CS和MISO都可以不要；
如果主机只读取从机送来的数据，SSEL和MOSI都可以不要。
